  ...
  def build_bundler
    instrument 'ruby.build_bundler' do
      log("bundle") do
        ...
        bundle_bin     = "bundle"
        bundle_command = "BUNDLE_GEMFILE=#{File.expand_path(env('BUNDLE_GEMFILE'))} #{bundle_bin} install --without #{bundle_without} --path vendor/bundle --binstubs #{bundler_binstubs_path}"
        bundle_command << " -j4"
